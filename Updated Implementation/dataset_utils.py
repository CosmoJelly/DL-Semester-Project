...
def preprocess_path_with_contours(path):
    ...
    contour_vec = np.array([
        feats.get('perimeter', 0.0),
        feats.get('area', 0.0),
        feats.get('epsilon', 0.0),
...
def make_tf_dataset_from_folder(folder, batch_size=BATCH_SIZE, train_split=TRAIN_SPLIT, 
    ...
    if return_contours:
        # First pass: preprocess all images to cache contour features
        # This ensures contour features are available
        for p in filepaths:
            img = cv2.imread(p)
            if img is not None:
                segment_and_crop_cv(img, image_path=p)
        
        train_ds = train_ds.map(lambda p,l: tf_preprocess_with_contours(p, l, augment=True), num_parallel_calls=autotune)
        val_ds = val_ds.map(lambda p,l: tf_preprocess_with_contours(p, l, augment=False), num_parallel_calls=autotune)
        test_ds = test_ds.map(lambda p,l: tf_preprocess_with_contours(p, l, augment=False), num_parallel_calls=autotune)
    else:
        train_ds = train_ds.map(lambda p,l: tf_preprocess(p, l, augment=True), num_parallel_calls=autotune)
        val_ds = val_ds.map(lambda p,l: tf_preprocess(p, l, augment=False), num_parallel_calls=autotune)
        test_ds = test_ds.map(lambda p,l: tf_preprocess(p, l, augment=False), num_parallel_calls=autotune)
