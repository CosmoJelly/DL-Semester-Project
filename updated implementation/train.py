...
def compute_class_weights(labels):
    ...
    # Convert to native Python types for TensorFlow compatibility
    return {int(k): float(v) for k, v in zip(unique_labels, class_weights)}
    ...
    try:
        ...
        for layer in base_model.layers[-layers_to_unfreeze:]:
            if hasattr(layer, 'trainable'):
                ...
                unfrozen_count += 1
        print(f"Unfroze last {unfrozen_count} layers of base model (out of {total_layers} total)")
    except Exception as e:
        print(f"[WARN] Could not unfreeze layers: {e}. Continuing with frozen model.")
    return model
    ...
    if use_class_weight:
        ...
        for batch in train_ds:
            try:
                ...
                print(f"[WARN] Error collecting labels for class weights: {e}")
                continue
        if all_labels:
            class_weights = compute_class_weights(all_labels)
            print("Class weights:", class_weights)
        else:
            print("[WARN] No valid labels collected for class weights, skipping class weighting")
